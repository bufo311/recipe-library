...
import { db, recipesTable } from "./db.js";
import { z } from "zod";
import { scrapeRecipeFromUrl } from "./lib/recipeScraper.js";
import { convertIngredientToGrams } from "./lib/gramConverter.js";
import { getSupabaseClient, STORAGE_BUCKET } from "./lib/supabase.js";
...
const ListRecipesParams = z.object({
...
const UpdateRecipeBody = CreateRecipeBody.partial();

const RecipeIdParam = z.object({ id: z.coerce.number().int().positive() });

const ScrapeRecipeBody = z.object({ url: z.string().url() });
...
router.get("/recipes", async (req, res) => {
  try {
    const query = ListRecipesParams.safeParse(req.query);
    ...
    const conditions = [];
    if (params.search) conditions.push(ilike(recipesTable.title, `%${params.search}%`));
    if (params.category) conditions.push(eq(recipesTable.category, params.category));
    if (params.course) conditions.push(eq(recipesTable.course, params.course));
    if (params.cuisine) conditions.push(eq(recipesTable.cuisine, params.cuisine));
    if (params.attribute) {
      conditions.push(sql`${recipesTable.attribute} @> ${JSON.stringify([params.attribute])}::jsonb`);
    }
    ...
    res.json(recipes.map((r) => ({ ...r, attribute: (r.attribute as string[]) ?? [], createdAt: r.createdAt.toISOString() })));
  } catch (err) {
...
router.post("/recipes", async (req, res) => {
  try {
    ...
    if (!parsed.success) { res.status(400).json({ error: "Invalid recipe data" }); return; }
...
router.post("/recipes/scrape", async (req, res) => {
  try {
    ...
    if (!parsed.success) { res.status(400).json({ error: "Invalid request: url is required" }); return; }
...
router.get("/recipes/stats", async (req, res) => {
  try {
    const [[{ total }], categoryCounts, recentResult] = await Promise.all([
      ...
      db.select({ category: recipesTable.category, count: count() }).from(recipesTable)
        .where(sql`${recipesTable.category} IS NOT NULL`).groupBy(recipesTable.category),
      db.select({ count: count() }).from(recipesTable)
        .where(sql`${recipesTable.createdAt} > NOW() - INTERVAL '7 days'`),
    ]);
    res.json({
      totalRecipes: total,
      categoryCounts: categoryCounts.map((c) => ({ category: c.category ?? "Uncategorized", count: c.count })),
...
router.get("/recipes/facets", async (req, res) => {
  try {
    const rows = await db.select({ course: recipesTable.course, cuisine: recipesTable.cuisine, attribute: recipesTable.attribute }).from(recipesTable);
...
router.get("/recipes/recent", async (req, res) => {
  try {
    ...
    const recipes = await db.select({
      id: recipesTable.id, title: recipesTable.title, imagePath: recipesTable.imagePath,
      yields: recipesTable.yields, category: recipesTable.category, sourceUrl: recipesTable.sourceUrl,
      course: recipesTable.course, cuisine: recipesTable.cuisine, attribute: recipesTable.attribute,
      createdAt: recipesTable.createdAt,
    }).from(recipesTable).orderBy(desc(recipesTable.createdAt)).limit(limit);
    res.json(recipes.map((r) => ({ ...r, attribute: (r.attribute as string[]) ?? [], createdAt: r.createdAt.toISOString() })));
  } catch (err) {
...
router.get("/recipes/:id", async (req, res) => {
  try {
    const parsed = RecipeIdParam.safeParse(req.params);
    if (!parsed.success) { res.status(400).json({ error: "Invalid recipe ID" }); return; }
    const [recipe] = await db.select().from(recipesTable).where(eq(recipesTable.id, parsed.data.id)).limit(1);
...
router.put("/recipes/:id", async (req, res) => {
  try {
    const paramsParsed = RecipeIdParam.safeParse(req.params);
    if (!paramsParsed.success) { res.status(400).json({ error: "Invalid recipe ID" }); return; }

    const bodyParsed = UpdateRecipeBody.safeParse(req.body);
    if (!bodyParsed.success) { res.status(400).json({ error: "Invalid recipe data" }); return; }

    const data = bodyParsed.data;
    const updates: Partial<typeof recipesTable.$inferInsert> = { updatedAt: new Date() };
    ...
    const [recipe] = await db.update(recipesTable).set(updates).where(eq(recipesTable.id, paramsParsed.data.id)).returning();
    if (!recipe) { res.status(404).json({ error: "Recipe not found" }); return; }
    res.json(serializeRecipe(recipe));
  } catch (err) {
...
router.delete("/recipes/:id", async (req, res) => {
  try {
    const parsed = RecipeIdParam.safeParse(req.params);
    if (!parsed.success) { res.status(400).json({ error: "Invalid recipe ID" }); return; }
    const [deleted] = await db.delete(recipesTable).where(eq(recipesTable.id, parsed.data.id)).returning({ id: recipesTable.id });
...
router.post("/recipes/:id/convert-to-grams", async (req, res) => {
  try {
    ...
    res.json({ originalIngredients: recipe.ingredients, convertedIngredients });
  } catch (err) {
...
router.post("/uploads", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (error) { req.log.error({ err: error }, "Supabase upload failed"); res.status(500).json({ error: "Image upload failed" }); return; }
