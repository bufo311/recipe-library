...
import { useGetRecipeFacets } from "@/lib/api-client";
...
export function RecipeForm({ defaultValues, onSubmit, isSubmitting }: RecipeFormProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: err instanceof Error ? err.message : "Failed to upload image", variant: "destructive" });
    } finally {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12">
        <div className="space-y-6">
          <FormField control={form.control} name="title" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input placeholder="e.g. Grandma's Apple Pie" className="text-xl font-serif h-14" {...field} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="sourceUrl" render={({ field }) => (
              <FormItem><FormLabel>Source URL (Optional)</FormLabel><FormControl><Input placeholder="https://..." {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="category" render={({ field }) => (
              <FormItem><FormLabel>Category (Optional)</FormLabel><FormControl><Input placeholder="e.g. Dessert, Dinner" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <FormField control={form.control} name="cookTime" render={({ field }) => (
              <FormItem><FormLabel>Cook Time</FormLabel><FormControl><Input placeholder="e.g. 45 mins" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="totalTime" render={({ field }) => (
              <FormItem><FormLabel>Total Time</FormLabel><FormControl><Input placeholder="e.g. 1 hour" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="yields" render={({ field }) => (
              <FormItem><FormLabel>Yields</FormLabel><FormControl><Input placeholder="e.g. 4 servings" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <FormItem>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <FormField control={form.control} name="course" render={({ field }) => (
              <FormItem><FormLabel>Course</FormLabel><FormControl>
                <CreatableCombobox options={facets?.courses ?? []} value={field.value || ""} onChange={field.onChange} placeholder="e.g. Dinner, Dessert" />
            ...
            <FormField control={form.control} name="attribute" render={({ field }) => (
              <FormItem><FormLabel>Attributes</FormLabel><FormControl>
                <AttributeMultiSelect options={facets?.attributes ?? []} value={field.value || []} onChange={field.onChange} placeholder="e.g. Quick, Vegetarian" />
              </FormControl><FormMessage /></FormItem>
            )} />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {form.watch("ingredients").map((_, index) => (
              <FormField key={index} control={form.control} name={`ingredients.${index}.value`} render={({ field }) => (
                <FormItem>
                  <div className="flex items-center gap-2">
                    <FormControl><Input placeholder="e.g. 2 cups all-purpose flour" {...field} /></FormControl>
                    <Button type="button" variant="ghost" size="icon" onClick={() => {
                      const items = form.getValues("ingredients");
                      if (items.length > 1) form.setValue("ingredients", items.filter((_, i) => i !== index));
                    }}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive transition-colors" /></Button>
                  </div>
                  <FormMessage />
                </FormItem>
              )} />
          ...
          <Button type="button" variant="outline" onClick={() => form.setValue("ingredients", [...form.getValues("ingredients"), { value: "" }])}>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {form.watch("instructions").map((_, index) => (
              <FormField key={index} control={form.control} name={`instructions.${index}.value`} render={({ field }) => (
                <FormItem>
                  <div className="flex gap-4">
                    <div className="flex-none pt-2 text-xl font-serif text-muted-foreground font-medium">{(index + 1).toString().padStart(2, "0")}</div>
                    <div className="flex-1 flex gap-2">
                      <FormControl><Textarea placeholder="Step instructions..." className="min-h-[80px]" {...field} /></FormControl>
                      <Button type="button" variant="ghost" size="icon" onClick={() => {
                        const items = form.getValues("instructions");
                        if (items.length > 1) form.setValue("instructions", items.filter((_, i) => i !== index));
                      }}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive transition-colors" /></Button>
                    </div>
                  </div>
                  <FormMessage />
          ...
          <Button type="button" variant="outline" onClick={() => form.setValue("instructions", [...form.getValues("instructions"), { value: "" }])}>
        ...
        <div className="space-y-6">
          ...
          <FormField control={form.control} name="notes" render={({ field }) => (
            <FormItem><FormControl><Textarea placeholder="Any additional notes, tips, or variations..." className="min-h-[120px]" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
